...
Class DB {
    ...
    public function __construct($option) {
        ...
        $this->connect($db_config["server"], $db_config["username"], $db_config["password"], $db_config["database_name"]);
	...
	//查询一条数据
    public function select_one($tab,$column = "*",$condition = '')   //查询函数
    {
    ...
    //查询多条数据
    public function select_all($tab,$column = "*",$condition = '')   //查询函数
    {
        $condition = $condition ? ' where ' . $condition : NULL;
        $sql = "select $column from $tab $condition";
    ...
    //获取一条记录（MYSQL_ASSOC，MYSQL_NUM，MYSQL_BOTH）              
    public function get_one_record($table,$condition,$result_type = MYSQL_ASSOC) {
    ...
    //获取全部记录
    public function get_all_record($table,$condition,$result_type = MYSQL_ASSOC) {
        $sql = "select * from ".$table." where ".$condition;
        $query = $this->query($sql);
        $i = 0;
        $rt = array();
